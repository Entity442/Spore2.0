    ...
    public void registerGoals() {
        ...
        this.goalSelector.addGoal(5,new AerialRangedGoal(this,1.3,40,16,5,10){
            ...
            public boolean canUse() {
                return super.canUse() && (this.target != null && (this.target.isOnGround() || this.target.isInFluidType()));
            }
        });
    ...
    public void performRangedAttack(LivingEntity livingEntity, float p_33318_) {
        if(!level.isClientSide){
            ...
            if (random.nextFloat() < 0.3f){
                ...
                vec3 = (new Vec3(0D, -2D, 0D)).yRot(-this.getYRot() * ((float)Math.PI / 180F) - ((float)Math.PI / 2F));
            }
