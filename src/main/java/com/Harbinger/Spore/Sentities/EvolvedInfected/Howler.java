...
public class Howler extends EvolvedInfected {
    ...
    protected SoundEvent getStepSound() { return SoundEvents.ZOMBIE_STEP; }
     @Override
    protected void playStepSound(BlockPos p_20135_, BlockState p_20136_) {this.playSound(getStepSound(), 0.15F, 1.0F);}
    ...
    public void SummonScream(LivingEntity caster) {
        ...
        if (entityType != null) {
            ...
            if (summoned != null) {
                summoned.randomTeleport(caster.getX() + dx, caster.getY() + 0.5D + dy, caster.getZ() + dz,false);
                summoned.finalizeSpawn(
                        levelAccessor,
                        level.getCurrentDifficultyAt(caster.getOnPos()),
