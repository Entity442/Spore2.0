...
public class Infected extends Monster{
    ...
    public static boolean checkMonsterInfectedRules(EntityType<? extends Infected> p_219014_, ServerLevelAccessor levelAccessor, MobSpawnType p_219016_, BlockPos pos, RandomSource source) {
        ...
        if (SConfig.SERVER.daytime_spawn.get()) {return  levelAccessor.getDifficulty() != Difficulty.PEACEFUL && checkMobSpawnRules(p_219014_, levelAccessor, p_219016_, pos, source) && (isDarkEnoughToSpawn(levelAccessor, pos, source) || levelAccessor.canSeeSky(pos));}
