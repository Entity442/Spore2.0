...
public abstract class BaseInfectedRenderer<T extends Mob, M extends EntityModel<T>> extends MobRenderer<T, M> {
    ...
    public void renderIllusions(LivingEntity living,Infected infected, float value2, PoseStack stack, MultiBufferSource source, int light){
        ...
        float f5 = 0.0F;
        if (infected.isAlive()) {
            f8 = Mth.lerp(value2, infected.animationSpeedOld, infected.animationSpeed);
            f5 = infected.animationPosition - infected.animationSpeed * (1.0F - value2);
        ...
        if (living != null){
            stack.pushPose();
            stack.mulPose(Vector3f.YP.rotationDegrees(-infected.yBodyRot));
