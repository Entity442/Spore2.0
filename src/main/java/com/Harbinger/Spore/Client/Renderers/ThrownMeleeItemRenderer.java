...
public class ThrownMeleeItemRenderer<T extends ThrownItemProjectile>extends EntityRenderer<T> {
    ...
    public void render(T entity, float value1, float value2, PoseStack stack, MultiBufferSource source, int lightValue) {
        if (!(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(entity) < 12.25)){
            stack.pushPose();
            stack.mulPose(Vector3f.YP.rotationDegrees(Mth.lerp(value2, entity.yRotO, entity.getYRot()) - 270.0F));
            stack.mulPose(Vector3f.ZP.rotationDegrees(Mth.lerp(value2, entity.xRotO, entity.getXRot()) + 90.0F));
            stack.scale(1.2f,1.2f,1.2f);
            this.itemRenderer.renderStatic(entity.getItem(), ItemTransforms.TransformType.GROUND, lightValue, OverlayTexture.NO_OVERLAY, stack, source, entity.getId());
            stack.popPose();
        }
